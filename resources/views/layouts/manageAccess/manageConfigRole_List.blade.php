    ...
    <div class="row">
        <div class="card p-0 b-1 shadow-sm border-light">
            <div class="card-header bg-light-info border-secondary px-3 pt-3 pb-2">
                <h4 class="card-title m-auto px-2 pb-1"><strong>Manage Config Role List</strong></h4>
            </div>
            <div class="card-body py-3 px-3 min-vh-50">
                <div class="row px-2 d-flex justify-content-start">
                    <div class="col-lg-2 col-md-3 col-sm-6">
                        <a href="{{ route('manageAccess.configRole.add') }}" class="btn btn-primary width-min-120">
                            ...
                            <table class="table table-responsive table-bordered table-hover table-striped w-100 text-sm" id="myTable">
                                <thead class="bg-light">
                                    <tr class="table-light">
                                        ...
                                        <th class="text-center align-middle width-min-120">Role Status</th>
                                ...
                                <tbody>
                                    ...
                                    @foreach ($data as $item)
                                        <tr>
                                            ...
                                            <td class="text-left align-middle">{{ $item['ROLE_TYPE'] ?? 'N/A' }}</td>
                                            <td class="text-left align-middle">{{ $item['ROLE_CATEGORY'] ?? 'N/A' }}</td>
                                            <td class="text-left align-middle">{{ $item['ROLE_NAME'] ?? 'N/A' }}</td>
                                            <td class="text-center align-middle">
                                                {!! $item['ROLE_STATUS'] === 'A' ? '<span class="mb-1 badge bg-success rounded-pill text-sm">ACTIVE</span>' 
                                                : ($item['ROLE_STATUS'] === 'N' ? '<span class="mb-1 badge bg-danger rounded-pill text-sm">NON ACTIVE</span>' 
                                            ...
                                            <td class="text-center align-middle">
                                                <a href="{{ route('manageAccess.configRole.detail', ['id' => $item['ROLE_ID']]) }}" 
